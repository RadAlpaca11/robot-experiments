...
def collectData(
        ...
        modelType: str = 'MJCF',
        jnt_names: list = None,
        endEffector: str = None,
        gripper: bool = True,
        gripperOpenPos: torch.Tensor = None,
        gripperClosePos: torch.Tensor = None,
        joint_weights: dict = None,
        joint_upperRange: dict = None,
        joint_lowerRange: dict = None,
        output_file: str = 'lerobotTests/robotData.parquet',
        repo: str = 'RadAlpaca11/lerobotTests',
        repo_output_path: str = 'robotData.parquet',
        commit_message: str = 'Add robot data from code',
        cube: bool = False,
        **kwargs
):
    
    modelType = modelType.upper()
    if modelType not in ['MJCF', 'URDF']:
        raise ValueError("modelType must be either 'MJCF' or 'URDF'")
    ...
    if modelType == 'MJCF':
        arm = scene.add_entity(
            gs.morphs.MJCF(file=modelPath),
        )
    elif modelType == 'URDF':
        arm = scene.add_entity(
            gs.morphs.URDF(file=modelPath),
        )
    ...
    end_effector = arm.get_link(endEffector)
    ...
    def generateJointPoints(totalPoints: int, weights: Dict[str, float], joints: List[str], lowerRange: Dict[str, float], upperRange: Dict[str, float]) -> List[Tuple[float, ...]]:
        ...
        changes_per_joint = {
            joint: max(1, int(round(weight* totalPoints)))
    ...
    api.upload_file(
        ...
        path_in_repo='smallerRobotDataTest.parquet',
