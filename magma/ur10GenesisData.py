...
ur10 = scene.add_entity(
    gs.morphs.MJCF(file='../genesis/mujoco_menagerie/universal_robots_ur10e/ur10e.xml'),
...
weights = {
    ...
    'j4': 0.15,
    'j5': 0.10,
    'j6': 0.00 # this is where a gripper would be, so rotating it is kinda useless right now since it does nothing
}

lowerRange = {
    'j1': -6.28,  # Joint 1 range
    'j2': -6.28,  # Joint 2 range
    'j3': -3.14,  # Joint 3 range
    'j4': -6.28,  # Joint 4 range
    'j5': -6.28,  # Joint 5 range
    'j6': -6.28,  # Joint 6 range
}
...
ep = 0
# zero positions
zeroPos = torch.Tensor([0, 0, 0, 0, 0, 0])
...
camFilm.stop_recording(save_to_filename='lerobotTests/picsAndVids/ur10/video.mp4')
# cam.stop_recording(save_to_filename='robotCam.mp4')

for env in range(envNum):
    currentPos = ur10.get_dofs_position(dofs_idx, [env])
    ...
    if env == 0:
        ...
        frame = getFrame(cam)
        cv2.imwrite('lerobotTests/picsAndVids/ur10/ep' + str(env) + '.jpg', frame)
        image = np.append(image, 'lerobotTests/picsAndVids/ur10/ep' + str(env) + '.jpg')
...
pq.write_table(table, 'lerobotTests/ur10RobotDataTest.parquet')

# Uploads the file to huggingface
api.upload_file(
    path_or_fileobj='lerobotTests/ur10RobotDataTest.parquet',
    path_in_repo='ur10RobotDataTest.parquet',
