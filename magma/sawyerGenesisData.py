...
sawyer = scene.add_entity(
    gs.morphs.MJCF(file='../genesis/mujoco_menagerie/rethink_robotics_sawyer/sawyer.xml')
...
dofs_idx = [sawyer.get_joint(name).dof_idx_local for name in jnt_names]
...
end_effector = sawyer.get_link('right_l6')
...
lowerRange = {
    'j0': -3.05,  # Joint 1 range
    'j1': -3.81,  # Joint 2 range
    'j2': -3.04,  # Joint 3 range
    'j3': -3.04,  # Joint 4 range
    'j4': -2.98,  # Joint 5 range
    'j5': -2.98,  # Joint 6 range
    'j6': -4.71,  # Joint 7 range
}

upperRange = {
    'j0': 3.05,  # Joint 1 range
    'j1': 2.27,  # Joint 2 range
    'j2': 3.04,  # Joint 3 range
    'j3': 3.04,  # Joint 4 range
    'j4': 2.98,  # Joint 5 range
    'j5': 2.98,  # Joint 6 range
...
for env in range(envNum):
    currentPos = sawyer.get_dofs_position(dofs_idx, [env])
